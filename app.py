...
def get_tw_stock_list():
    """抓上市與上櫃清單，用於自動判斷 .TW / .TWO。若失敗，主程式仍會自動 fallback 測試兩種 suffix。"""
    stock_map = {}
...
def choose_and_fetch(code, market_mode, period, stock_map):
    """依市場選擇抓資料；自動模式下若 .TW 失敗會自動試 .TWO，反之亦然。"""
    candidates = []

    if market_mode == "上市 .TW":
        candidates = [(f"{code}.TW", code, "上市")]
    elif market_mode == "上櫃 .TWO":
        candidates = [(f"{code}.TWO", code, "上櫃")]
    else:
        if code in stock_map:
            info = stock_map[code]
            primary = info["ticker"]
            if primary.endswith(".TW"):
                candidates = [(primary, info["name"], info["market"]), (f"{code}.TWO", info["name"], "上櫃 fallback")]
            else:
                candidates = [(primary, info["name"], info["market"]), (f"{code}.TW", info["name"], "上市 fallback")]
        else:
            candidates = [(f"{code}.TW", code, "上市 fallback"), (f"{code}.TWO", code, "上櫃 fallback")]
    ...
    return candidates[-1][0], code, "抓取失敗", pd.DataFrame(), attempts
...
if submitted:
    ...
    st.subheader(f"{code} {name}｜{market}｜{ticker}")
